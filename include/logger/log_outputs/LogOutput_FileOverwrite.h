...
namespace eadlib {
    namespace log {
        class LogOutput_FileOverwrite : public LogOutput {
          public:
            LogOutput_FileOverwrite() : _output_stream( std::make_unique<std::ofstream>() ) { }
        ...
        inline void eadlib::log::LogOutput_FileOverwrite::close_ostream() {
            if( _output_stream ) {
                ...
                if( _output_stream->fail() ) {
                    throw( std::runtime_error( "eadlib::Logger::LogOutput_FileOverwrite: Didn't close the output stream properly" ) );
                }
            }
        }
